...
/* =========================================================
   EXPORTAÇÃO — PDF / PNG (compatível com iPhone/iPad)

   Importante: não usamos window.print() nem <a download> com data:.
   No iOS esses mecanismos podem não disparar nada. Em vez disso:
   - geramos o arquivo localmente;
   - no computador fazemos download normal;
   - no iPhone/iPad abrimos o arquivo em uma nova aba, onde o Safari
     permite usar Compartilhar -> Salvar em Arquivos / Salvar Imagem.
========================================================= */
...
function openExportWindow(title, mime) {
  // Abre durante o clique do usuário. Isso evita que o bloqueador de pop-up
  // do Safari rejeite a nova aba depois que a geração assíncrona terminar.
  if (!isIOSLike()) return null;
  const w = window.open('', '_blank');
  if (!w) return null;
  try {
    w.document.open();
    w.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>html,body{margin:0;padding:0;background:#f2f3f7;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;height:100%;}body{display:flex;align-items:center;justify-content:center;}p{padding:20px;color:#555;text-align:center;}img{max-width:100%;height:auto;display:block;margin:auto;}iframe{border:0;width:100%;height:100vh;background:#fff;}</style></head><body><p>Gerando arquivo…</p></body></html>`);
    w.document.close();
  } catch (_) {}
  return w;
}

function deliverBlob(blob, filename, kind, openedWindow) {
  const url = URL.createObjectURL(blob);

  if (openedWindow && !openedWindow.closed) {
    try {
      if (kind === 'png') {
        openedWindow.document.body.innerHTML = `<img src="${url}" alt="Recibo">`;
      } else {
        openedWindow.document.body.innerHTML = `<iframe src="${url}" title="Recibo em PDF"></iframe>`;
      }
      openedWindow.document.title = filename;
      // Não revogamos imediatamente: o Safari ainda precisa consumir o blob.
      setTimeout(() => URL.revokeObjectURL(url), 10 * 60 * 1000);
      return true;
    } catch (_) {}
  }

  // Fallback para navegadores de desktop e navegadores iOS que não abriram aba.
...
/*
 * Cria um PDF de uma página contendo o PNG/JPEG do recibo.
 * Não depende de bibliotecas externas: funciona totalmente offline.
 * O JPEG é embutido diretamente no PDF, então o resultado preserva
 * o desenho do recibo produzido pelo canvas.
 */
function jpegDataUrlToPdfBlob(dataUrl, pixelWidth, pixelHeight) {
  const jpeg = base64ToUint8Array(dataUrl);

  // Tamanho da página em pontos, mantendo a proporção do recibo.
  // Largura máxima de 190 mm (~538.58 pt), com margens de 10 mm.
  const maxPageW = 538.58;
  const maxPageH = 790;
  ...
  objects[3] = `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pageW.toFixed(2)} ${pageH.toFixed(2)}] /Resources << /XObject << /${imageName} 5 0 R >> >> /Contents 4 0 R >>`;
  objects[4] = `<< /Length ${content.length} >>\nstream\n${content}endstream`;
  objects[5] = `<< /Type /XObject /Subtype /Image /Width ${pixelWidth} /Height ${pixelHeight} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpeg.length} >>\nstream\n`;
...
document.getElementById('btnBaixarPDF').addEventListener('click', async () => {
  const d = getCurrentReciboData();
  const openedWindow = openExportWindow(`Recibo #${d.numero}`, 'application/pdf');
  ...
  try {
    ...
    // JPEG é usado dentro do PDF para reduzir bastante o tamanho do arquivo.
    const jpegDataUrl = canvas.toDataURL('image/jpeg', 0.94);
    const pdfBlob = jpegDataUrlToPdfBlob(jpegDataUrl, canvas.width, canvas.height);
    deliverBlob(pdfBlob, `recibo-${d.numero}.pdf`, 'pdf', openedWindow);
  } catch (err) {
    if (openedWindow && !openedWindow.closed) openedWindow.close();
    console.error('Erro ao gerar PDF:', err);
    alert('Não foi possível gerar o PDF neste momento. Tente novamente.');
  } finally {
...
document.getElementById('btnBaixarPNG').addEventListener('click', async () => {
  const d = getCurrentReciboData();
  const openedWindow = openExportWindow(`Recibo #${d.numero}`, 'image/png');
  const btn = document.getElementById('btnBaixarPNG');
  const original = btn.textContent;
  ...
  try {
    ...
    deliverBlob(blob, `recibo-${d.numero}.png`, 'png', openedWindow);
  } catch (err) {
    if (openedWindow && !openedWindow.closed) openedWindow.close();
    console.error('Erro ao gerar PNG:', err);
    alert('Não foi possível gerar o PNG neste momento. Tente novamente.');
  } finally {
    btn.textContent = original;
    btn.style.pointerEvents = '';
  }
